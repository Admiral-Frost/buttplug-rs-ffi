...
use buttplug::{
    connector::{
        transport::{
            ButtplugConnectorTransport, ButtplugConnectorTransportConnectResult,
            ButtplugConnectorTransportSpecificError, ButtplugTransportMessage,
        },
                ...
                while let Ok(msg) = recvr_clone.recv().await {
                    match msg {
                        ButtplugSerializedMessage::Text(text) => {
                            wscs.send_with_str(&text).unwrap()
                        } //Message::Text(text),
